...
use client_sdk_schema as schema;
...
pub fn generate_resource_module(
    ...
    match resource {
        schema::Resource::SubResources(index_map) => {
            ...
            write_generated_ts_file(current_path.join("index.ts"), index_ts_content).unwrap();
        }
        schema::Resource::Parameter(parameter) => {
            let current_module_path = path.join(format!("{resource_name}.ts"));
...
pub(crate) fn write_generated_ts_file<P: AsRef<Path>>(
    path: P,
    content: String,
) -> std::io::Result<()> {
